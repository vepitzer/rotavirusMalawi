...
load('malawi_mcmc_outputV_nr.mat','keep_pars_V_nr')
...
avacc=[1 3 3 3; %1=birth, 2=1mo, 3=2mo, etc
       ...
       0 0 0 10]; %[1 5 9]
sc1=betarnd(19,17); 
sc2=betarnd(32.2,3.8); 
sc3=betarnd(32.2,3.8); 
%sc1=keep_pars_V_nr(10*samp-5000*floor((samp-.5)/500),1,ceil((samp-.5)/500)); %.528; %vpfit1_mal(1); 
%sc2=keep_pars_V_nr(10*samp-5000*floor((samp-.5)/500),2,ceil((samp-.5)/500)); %.895; %vpfit1_mal(2); 
%sc3=keep_pars_V_nr(10*samp-5000*floor((samp-.5)/500),2,ceil((samp-.5)/500)); %.895; %vpfit1_mal(2); 
...
Hsamp0_novacc_nr(:,samp)=sum(Hnovacc(:,1:12),2);
Hsamp1_novacc_nr(:,samp)=sum(Hnovacc(:,13:au2),2);
Hsamp2_novacc_nr(:,samp)=sum(Hnovacc(:,au2+1:au5),2);
if targ==3
Hsamp0_nr(:,samp)=sum(H(:,1:12),2);
Hsamp1_nr(:,samp)=sum(H(:,13:au2),2);
Hsamp2_nr(:,samp)=sum(H(:,au2+1:au5),2);
elseif targ==4
Hsamp0_boost_nr(:,samp)=sum(H(:,1:12),2);
Hsamp1_boost_nr(:,samp)=sum(H(:,13:au2),2);
Hsamp2_boost_nr(:,samp)=sum(H(:,au2+1:au5),2);
end

VEtot_samp_nr(samp,1)=1-((sum(Hv(tvacc+52:tvf-t0,4:au5))./sum(V(tvacc+52:tvf-t0,4:au5)))*agedistV(4:au5)')/((sum(Hu(tvacc+52:tvf-t0,4:au5))./sum(U(tvacc+52:tvf-t0,4:au5)))*agedistU(4:au5)');
VEage_samp_nr(samp,:)=1-[(sum(sum(Hv(tvacc+52:tvf-t0,4:12),2))/sum(sum(V(tvacc+52:tvf-t0,4:12),2)))/(sum(sum(Hu(tvacc+52:tvf-t0,4:12),2))/sum(sum(U(tvacc+52:tvf-t0,4:12),2)))...
...
%IEtot_samp_nr(samp,1)=1-((sum(Hu(tvacc+52:tvf-t0,1:au5))./sum(U(tvacc+52:tvf-t0,1:au5)))*agedistU(1:au5)')/((sum(Hnovacc(tvacc+52:tvf-t0,1:au5))./sum(Unovacc(tvacc+52:tvf-t0,1:au5)))*agedist_prevacc(1:au5)');
IEtot_samp_nr(samp,1)=sum((1-(sum(Hu(tvacc+52:tvf-t0,1:au5).*Hu(tvacc+52:tvf-t0,1:au5)./U(tvacc+52:tvf-t0,1:au5),2)./sum(Hu(tvacc+52:tvf-t0,1:au5),2))./(sum(Hnovacc(tvacc+52:tvf-t0,1:au5).*Hnovacc(tvacc+52:tvf-t0,1:au5)./Unovacc(tvacc+52:tvf-t0,1:au5),2)./sum(Hnovacc(tvacc+52:tvf-t0,1:au5),2))).*sum(Hu(tvacc+52:tvf-t0,1:au5),2))/sum(sum(Hu(tvacc+52:tvf-t0,1:au5),2));
IEage_samp_nr(samp,:)=1-[(sum(sum(Hu(tvacc+52:tvf-t0,1:12),2))/sum(sum(U(tvacc+52:tvf-t0,1:12),2)))/(sum(sum(Hnovacc(tvacc+52:tvf-t0,1:12),2))/sum(sum(Unovacc(tvacc+52:tvf-t0,1:12),2)))...
...
VEtot_boostsamp_nr(samp,1)=1-((sum(Hv(tvacc+315:tvacc+419,4:au5))./sum(V(tvacc+315:tvacc+419,4:au5)))*agedistV(4:au5)')/((sum(Hu(tvacc+315:tvacc+419,4:au5))./sum(U(tvacc+315:tvacc+419,4:au5)))*agedistU(4:au5)');
VEage_boostsamp_nr(samp,:)=1-[(sum(sum(Hv(tvacc+315:tvacc+419,4:12),2))/sum(sum(V(tvacc+315:tvacc+419,4:12),2)))/(sum(sum(Hu(tvacc+315:tvacc+419,4:12),2))/sum(sum(U(tvacc+315:tvacc+419,4:12),2)))...
...
%sampled_logLL_Vnfnr(samp,1)=rotamalV_LL(St(tvacc+1:tvf-t0,:),pars,rotamalVS3_vacc,rotamalVS3_unvacc,repeffV3,time(tvacc+1:tvf-t0,1));
...
%%
Hsamp_novacc_nr=Hsamp0_novacc_nr+Hsamp1_novacc_nr+Hsamp2_wane_nr;
Hsamp_nr=Hsamp0_nr+Hsamp1_nr+Hsamp2_nr;

Hobserr_nr=zeros(tmax-t0,10000);
Hobserr0_nr=zeros(tmax-t0,10000);
Hobserr1_nr=zeros(tmax-t0,10000);
Hobserr2_nr=zeros(tmax-t0,10000);
for i=1:100
    Hobserr_nr(:,100*(i-1)+1:100*i)=poissrnd(Hsamp_nr);
    Hobserr0_nr(:,100*(i-1)+1:100*i)=poissrnd(Hsamp0_nr);
    Hobserr1_nr(:,100*(i-1)+1:100*i)=poissrnd(Hsamp1_nr);
    Hobserr2_nr(:,100*(i-1)+1:100*i)=poissrnd(Hsamp2_nr);
end
Hpi_nr=[prctile(Hobserr_nr,2.5,2) prctile(Hobserr_nr,97.5,2)];
Hpi0_nr=[prctile(Hobserr0_nr,2.5,2) prctile(Hobserr0_nr,97.5,2)];
Hpi1_nr=[prctile(Hobserr1_nr,2.5,2) prctile(Hobserr1_nr,97.5,2)];
Hpi2_nr=[prctile(Hobserr2_nr,2.5,2) prctile(Hobserr2_nr,97.5,2)];

%%
outside_pi=zeros(520,4);
for i=1:431 
    if sum(rotamalPV(i,:),2)<Hpi_nr(tvacc+i,1) || sum(rotamalPV(i,:),2)>Hpi_nr(tvacc+i,2)
        outside_pi(i,1)=1;
    end
    if sum(rotamalPV(i,1:12),2)<Hpi0_nr(tvacc+i,1) || sum(rotamalPV(i,1:12),2)>Hpi0_nr(tvacc+i,2)
        outside_pi(i,2)=1;
    end
    if sum(rotamalPV(i,13:au2),2)<Hpi1_nr(tvacc+i,1) || sum(rotamalPV(i,13:au2),2)>Hpi1_nr(tvacc+i,2)
        outside_pi(i,3)=1;
    end
    if sum(rotamalPV(i,au2+1:au5),2)<Hpi2_nr(tvacc+i,1) || sum(rotamalPV(i,au2+1:au5),2)>Hpi2_nr(tvacc+i,2)
        outside_pi(i,4)=1;
    end
end
for i=459:547
...
%%
%r_samp_nr=zeros(1,100);
%r_pval_nr=zeros(1,100);
for i=1:100
    [r_samp_nr(i),r_pval_nr(i)]=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp_nr([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    r_samp_nr_early(i)=corr(sum(rotamalPV(1:295,:),2),Hsamp_nr(tvacc+1:tvacc+295,i),'Type','Spearman');
    r_samp_nr_late(i)=corr(sum(rotamalPV([296:431 459:end],:),2),Hsamp_nr([tvacc+296:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    r_samp0_nr(i)=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp0_nr([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    r_samp1_nr(i)=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp1_nr([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    r_samp2_nr(i)=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp2_nr([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
end
mean([r_samp_nr' r_pval_nr'])
mean([r_samp_nr_early' r_samp_nr_late'])
%prctile(r_samp_nr',[2.5 97.5])
%mean([r_samp0_nr' r_samp1_nr' r_samp2_nr'])

%%
save('malawi_pvsamp_output_nr.mat','Hsamp0_nr','Hsamp1_nr','Hsamp2_nr','Hsamp0_boost_nr','Hsamp1_boost_nr','Hsamp2_boost_nr',...
    'Hsamp0_novacc_nr','Hsamp1_novacc_nr','Hsamp2_novacc_nr',...
    'VEtot_samp_nr','VEage_samp_nr','IEtot_samp_nr','IEage_samp_nr','VEtot_boostsamp_nr','VEage_boostsamp_nr')
%%
datesim(1:t0,:)=[];
...
%%
%samp_posterior_nfnr=zeros(1000,1);
%for samp=1:1000
%    samp_posterior_nfnr(samp,1)=sampled_logLL_Vnfnr(samp,1)+log(betapdf(betarnd(19,17),19,17))+log(betapdf(betarnd(32.2,3.8),32.2,3.8));
%end
%model_bic_nfnr=log(295*6)*0-2*mean(samp_posterior_nfnr)
%model_delta_nr=model_bic_nr-model_bic_wane
