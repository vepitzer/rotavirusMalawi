...
load('malawi_mcmc_outputV.mat','keep_pars_V')
...
avacc=[1 3 3 3; %1=birth, 2=1mo, 3=2mo, etc
       0 0 4 4; 
       0 0 0 10]; %[1 5 9]
sc1=keep_pars_V(10*samp-5000*floor((samp-.5)/500),1,ceil((samp-.5)/500)); %.528; %.687; %vpfit1_mal(1); %.568; %betarnd(24.7,11.3); %.63; %betarnd(192,117); %
sc2=keep_pars_V(10*samp-5000*floor((samp-.5)/500),2,ceil((samp-.5)/500)); %.895; %.687; %vpfit1_mal(2); %.736; %betarnd(24.7,11.3); %%.63; %betarnd(192,117); %
sc3=keep_pars_V(10*samp-5000*floor((samp-.5)/500),2,ceil((samp-.5)/500)); %.895; %.687; %vpfit1_mal(2); %.736; %betarnd(55,25); %
...
wv=1/(52*keep_pars_V(10*samp-5000*floor((samp-.5)/500),3,ceil((samp-.5)/500))); %vpfit3_mal; %vpfit2_mal(3); %0; %1/78; %rate of waning of vaccine-induced immunity
...
Hsamp0_novacc_wane(:,samp)=sum(Hnovacc(:,1:12),2);
Hsamp1_novacc_wane(:,samp)=sum(Hnovacc(:,13:au2),2);
Hsamp2_novacc_wane(:,samp)=sum(Hnovacc(:,au2+1:au5),2);
if targ==3
...
%IEtot_samp_wane(samp,1)=1-((sum(Hu(tvacc+52:tvf-t0,1:au5))./sum(U(tvacc+52:tvf-t0,1:au5)))*agedistU(1:au5)')/((sum(Hnovacc(tvacc+52:tvf-t0,1:au5))./sum(Unovacc(tvacc+52:tvf-t0,1:au5)))*agedist_prevacc(1:au5)');
...
%%
Hsamp_novacc_wane=Hsamp0_novacc_wane+Hsamp1_novacc_wane+Hsamp2_novacc_wane;
Hsamp_wane=Hsamp0_wane+Hsamp1_wane+Hsamp2_wane;

Hobserr_wane=zeros(tmax-t0,10000);
Hobserr0_wane=zeros(tmax-t0,10000);
Hobserr1_wane=zeros(tmax-t0,10000);
Hobserr2_wane=zeros(tmax-t0,10000);
for i=1:100
    Hobserr_wane(:,100*(i-1)+1:100*i)=poissrnd(Hsamp_wane);
    Hobserr0_wane(:,100*(i-1)+1:100*i)=poissrnd(Hsamp0_wane);
    Hobserr1_wane(:,100*(i-1)+1:100*i)=poissrnd(Hsamp1_wane);
    Hobserr2_wane(:,100*(i-1)+1:100*i)=poissrnd(Hsamp2_wane);
end
Hpi_wane=[prctile(Hobserr_wane,2.5,2) prctile(Hobserr_wane,97.5,2)];
Hpi0_wane=[prctile(Hobserr0_wane,2.5,2) prctile(Hobserr0_wane,97.5,2)];
Hpi1_wane=[prctile(Hobserr1_wane,2.5,2) prctile(Hobserr1_wane,97.5,2)];
Hpi2_wane=[prctile(Hobserr2_wane,2.5,2) prctile(Hobserr2_wane,97.5,2)];

%%
outside_pi=zeros(520,4);
for i=1:431 
    if sum(rotamalPV(i,:),2)<Hpi_wane(tvacc+i,1) || sum(rotamalPV(i,:),2)>Hpi_wane(tvacc+i,2)
        outside_pi(i,1)=1;
    end
    if sum(rotamalPV(i,1:12),2)<Hpi0_wane(tvacc+i,1) || sum(rotamalPV(i,1:12),2)>Hpi0_wane(tvacc+i,2)
        outside_pi(i,2)=1;
    end
    if sum(rotamalPV(i,13:au2),2)<Hpi1_wane(tvacc+i,1) || sum(rotamalPV(i,13:au2),2)>Hpi1_wane(tvacc+i,2)
        outside_pi(i,3)=1;
    end
    if sum(rotamalPV(i,au2+1:au5),2)<Hpi2_wane(tvacc+i,1) || sum(rotamalPV(i,au2+1:au5),2)>Hpi2_wane(tvacc+i,2)
        outside_pi(i,4)=1;
    end
end
for i=459:547
...
%%
r_samp_wane=zeros(1,100);
r_pval_wane=zeros(1,100);
for i=1:100
    [r_samp_wane(i),r_pval_wane(i)]=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp_wane([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    r_samp_wane_early(i)=corr(sum(rotamalPV(1:295,:),2),Hsamp_wane(tvacc+1:tvacc+295,i),'Type','Spearman');
    r_samp_wane_late(i)=corr(sum(rotamalPV([296:431 459:end],:),2),Hsamp_wane([tvacc+296:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    [r_samp0_wane(i),r_pval0_wane(i)]=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp0_wane([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    [r_samp1_wane(i),r_pval1_wane(i)]=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp1_wane([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
    [r_samp2_wane(i),r_pval2_wane(i)]=corr(sum(rotamalPV([1:431 459:end],:),2),Hsamp2_wane([tvacc+1:tvacc+431 tvacc+459:(end-1)],i),'Type','Spearman');
end
mean([r_samp_wane' r_pval_wane'])
mean([r_samp_wane_early' r_samp_wane_late'])
prctile(r_samp_wane',[2.5 97.5])
mean([r_samp0_wane' r_samp1_wane' r_samp2_wane'])

%%
save('malawi_pvsamp_output_wane.mat','Hsamp0_wane','Hsamp1_wane','Hsamp2_wane','Hsamp0_boost_wane','Hsamp1_boost_wane','Hsamp2_boost_wane',...
    'Hsamp0_novacc_wane','Hsamp1_novacc_wane','Hsamp2_novacc_wane',...
...
title('Waning of vaccine-induced immunity (homogeneous response)')
...
%%
%samp_posterior_wane=zeros(1000,1);
%for samp=1:1000
%    samp_posterior_wane(samp,1)=sampled_logLL_V(500+10*samp-5000*floor((samp-.5)/500),ceil((samp-.5)/550))+log(betapdf(keep_pars_V(10*samp-5000*floor((samp-.5)/500),1,ceil((samp-.5)/500)),24.7,11.3))+log(betapdf(keep_pars_V(10*samp-5000*floor((samp-.5)/500),2,ceil((samp-.5)/500)),24.7,11.3));
%end
%model_delta_wane=model_bic_wane-model_bic_wane
