...
sampled_pars_V_bc=zeros(total_steps,npar,nchains); 
sampled_logLL_V_bc=zeros(total_steps,nchains); 
accept=zeros(total_steps,nchains);
accept_rate_V_bc=zeros(total_steps,nchains); 
keep_pars_V_bc=zeros(total_steps-burnin_steps,npar,nchains);
...
for i=1:total_steps
    ...
    sampled_pars_V_bc(i,:,chain)=[old_sc1 old_sc2];  
    sampled_logLL_V_bc(i,chain)=old_logLL;
end
end

% Calculate acceptance rate
for i=2:total_steps
    accept_rate_V_bc(i,:)=mean(accept(1:i,:));
end

% Only keep values after the burn-in period
for chain=1:2
    keep_pars_V_bc(:,:,chain)=sampled_pars_V_bc(burnin_steps+1:total_steps,:,chain);
end

save('malawi_mcmc_outputV_bc.mat','sampled_pars_V_bc','sampled_logLL_V_bc','accept_rate_V_bc','keep_pars_V_bc')

%%
figure
subplot(2,2,1); plot([sampled_pars_V_bc(:,1,1) sampled_pars_V_bc(:,1,2)]); hold on; plot([burnin_steps burnin_steps],[0 1],'--k'); xlabel('Iteration'); ylabel('\its_1'); xlim([0 6000])
subplot(2,2,2); plot([sampled_pars_V_bc(:,2,1) sampled_pars_V_bc(:,2,2)]); hold on; plot([burnin_steps burnin_steps],[0 1],'--k'); xlabel('Iteration'); ylabel('\its_2'); xlim([0 6000])
subplot(2,2,3); hist([keep_pars_V_bc(:,1,1) keep_pars_V_bc(:,1,2)]); xlabel('\its_1'); ylabel('No. of posterior samples')
subplot(2,2,4); hist([keep_pars_V_bc(:,2,1) keep_pars_V_bc(:,2,2)]); xlabel('\its_2'); ylabel('No. of posterior samples')

%%
figure
subplot(2,1,1); plot(sampled_logLL_V_bc)
subplot(2,1,2); plot(accept_rate_V_bc)

%% Gelman-Rubin diagnostic
x=var(keep_pars_V_bc(:,:,1));
x(2,:)=var(keep_pars_V_bc(:,:,2))
y=mean(keep_pars_V_bc(:,:,1));
y(2,:)=mean(keep_pars_V_bc(:,:,2))
