...
sampled_pars_Vnr=zeros(total_steps,npar,nchains); %[sampled_pars_Vnr; zeros(1000,npar,nchains)]; %
sampled_logLL_Vnr=zeros(total_steps,nchains); %[sampled_logLL_V; zeros(1000,nchains)]; %
accept=zeros(total_steps,nchains); %[accept; zeros(1000,nchains)]; %
accept_rate_Vnr=zeros(total_steps,nchains); %[accept_rate_V; zeros(1000,nchains)]; %
keep_pars_Vnr=zeros(total_steps-burnin_steps,npar,nchains);
...
for i=1:total_steps
    ...
    current_sc1=old_sc1+.05*randn; 
    log_prior_ratio=log_prior_ratio+log(betapdf(current_sc1,19,17))-log(betapdf(old_sc1,19,17)); 
    sc1=current_sc1;
    
    % Propose new value for sc2
    current_sc2=old_sc2+.05*randn; 
    log_prior_ratio=log_prior_ratio+log(betapdf(current_sc2,32.2,3.8))-log(betapdf(old_sc2,32.2,3.8)); 
    sc2=current_sc2; sc2n=(1-sc2)*sc1/(1-sc1); 
    ...
    sampled_pars_Vnr(i,:,chain)=[old_sc1 old_sc2 old_wv];  
    sampled_logLL_Vnr(i,chain)=old_logLL;
end
end

% Calculate acceptance rate
for i=2:total_steps
    accept_rate_Vnr(i,:)=mean(accept(1:i,:));
end

% Only keep values after the burn-in period
for chain=1:2
    keep_pars_Vnr(:,:,chain)=sampled_pars_Vnr(burnin_steps+1:total_steps,:,chain);
end

save('malawi_mcmc_outputVnr.mat','sampled_pars_Vnr','sampled_logLL_Vnr','accept_rate_Vnr','keep_pars_Vnr')

%%
figure
subplot(2,3,1); plot([sampled_pars_Vnr(:,1,1) sampled_pars_Vnr(:,1,2)]); hold on; plot([burnin_steps burnin_steps],[0 1],'--k'); xlabel('Iteration'); ylabel('\its_1'); xlim([0 6000])
subplot(2,3,2); plot([sampled_pars_Vnr(:,2,1) sampled_pars_Vnr(:,2,2)]); hold on; plot([burnin_steps burnin_steps],[0 1],'--k'); xlabel('Iteration'); ylabel('\its_2'); xlim([0 6000])
subplot(2,3,3); plot([sampled_pars_Vnr(:,3,1) sampled_pars_Vnr(:,3,2)]); hold on; plot([burnin_steps burnin_steps],[0 40],'--k'); xlabel('Iteration'); ylabel({'Duration of immunity';'(years)'}); xlim([0 6000])
subplot(2,3,4); hist([keep_pars_Vnr(:,1,1) keep_pars_Vnr(:,1,2)]); xlabel('\its_1'); ylabel('No. of posterior samples')
subplot(2,3,5); hist([keep_pars_Vnr(:,2,1) keep_pars_Vnr(:,2,2)]); xlabel('\its_2'); ylabel('No. of posterior samples')
subplot(2,3,6); hist([keep_pars_Vnr(:,3,1) keep_pars_Vnr(:,3,2)]); xlabel('Duration of immunity (years)'); ylabel('No. of posterior samples')

%%
figure
subplot(2,1,1); plot(sampled_logLL_Vnr)
subplot(2,1,2); plot(accept_rate_Vnr)

%% Gelman-Rubin diagnostic(?)
%x=var(keep_pars_Vnr(:,:,1));
%x(2,:)=var(keep_pars_Vnr(:,:,2))
%y=mean(keep_pars_Vnr(:,:,1));
%y(2,:)=mean(keep_pars_Vnr(:,:,2))

%grdiag_Vnr=sqrt(((run_steps-1)/run_steps*mean(x) + var(y))./mean(x))
