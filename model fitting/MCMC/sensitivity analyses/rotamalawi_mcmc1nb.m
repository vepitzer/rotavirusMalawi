% NOTE: This code takes up to a week to run

load('malawidata.mat')

clear global all
global B wm wi1 wi2 u um d1 d2 rr1 rr2 ri2 ri3 v1 v2 v3 reintro wA; %al beta b1 phi1 immig deaths b2 phi2 
...
npar=5; %Number of parameters to be estimated
nchains=2;
burnin_steps=1000;
run_steps=10000;
...
sampled_pars_1nb=zeros(total_steps,npar,nchains);
sampled_logLL_1nb=zeros(total_steps,nchains);
accept=zeros(total_steps,nchains);
accept_rate_1nb=zeros(total_steps,nchains);
keep_pars_1nb=zeros(total_steps-burnin_steps,npar,nchains);
...
nbr=old_nbr; %shape parameter for negative binomial distribution

pars=[ptrans; b1; phi1; al; 1; 1; h; nbr];
...
old_logLL=rotamal_LLnb(St,pars,rotamalT2,rotamal08T2,negtest_movavg,negtest08_movavg,time);
...
for i=1:total_steps
    ...
    % Propose new value for nbr
    current_nbr=old_nbr*exp(wr*randn);
    log_proposal_ratio=log_proposal_ratio+log(current_nbr)-log(old_nbr);
    log_prior_ratio=log_prior_ratio+log(unifpdf(current_nbr,0,1000))-log(unifpdf(old_nbr,0,1000)); %Uniform(0,1) prior (i.e. all values between 0 and 1 are equally likely)
    nbr=current_nbr;
    ...
    current_logLL=rotamal_LLnb(St,pars,rotamalT2,rotamal08T2,negtest_movavg,negtest08_movavg,time);
    ...
    sampled_pars_1nb(i,:,chain)=[old_ptrans old_b1 old_phi old_h old_nbr];  
    sampled_logLL_1nb(i,chain)=old_logLL;
end
end

% Calculate acceptance rate
for i=2:total_steps
    accept_rate_1nb(i,:)=mean(accept(1:i,:));
end

% Only keep values after the burn-in period
for chain=1:2
    keep_pars_1nb(:,:,chain)=sampled_pars_1nb(burnin_steps+1:total_steps,:,chain);
end

save('malawi_mcmc_output1nb.mat','sampled_pars_1nb','sampled_logLL_1nb','accept_rate_1nb','keep_pars_1nb')

%%
figure
subplot(2,4,1); plot([sampled_pars_1nb(:,1,1) sampled_pars_1nb(:,1,2)])
subplot(2,4,2); plot([sampled_pars_1nb(:,2,1) sampled_pars_1nb(:,2,2)])
subplot(2,4,3); plot([sampled_pars_1nb(:,3,1) sampled_pars_1nb(:,3,2)])
subplot(2,4,4); plot([sampled_pars_1nb(:,4,1) sampled_pars_1nb(:,4,2)])
subplot(2,4,5); hist([keep_pars_1nb(:,1,1) keep_pars_1nb(:,1,2)])
subplot(2,4,6); hist([keep_pars_1nb(:,2,1) keep_pars_1nb(:,2,2)])
subplot(2,4,7); hist([keep_pars_1nb(:,3,1) keep_pars_1nb(:,3,2)])
subplot(2,4,8); hist([keep_pars_1nb(:,4,1) keep_pars_1nb(:,4,2)])

%%
figure
subplot(2,1,1); plot(sampled_logLL_1nb)
subplot(2,1,2); plot(accept_rate_1nb)

%% Gelman-Rubin diagnostic(?)
run_steps=10000;
x=var(keep_pars_1nb(:,:,1));
x(2,:)=var(keep_pars_1nb(:,:,2))
y=mean(keep_pars_1nb(:,:,1));
y(2,:)=mean(keep_pars_1nb(:,:,2))

grdiag_1nb=sqrt(((run_steps-1)/run_steps*mean(x) + var(y))./mean(x))
