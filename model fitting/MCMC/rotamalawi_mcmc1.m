...
sampled_pars_1=zeros(total_steps,npar,nchains);
sampled_logLL_1=zeros(total_steps,nchains);
accept=zeros(total_steps,nchains);
accept_rate_1=zeros(total_steps,nchains);
keep_pars_1=zeros(total_steps-burnin_steps,npar,nchains);
...
for i=1:total_steps
    ...
    sampled_pars_1(i,:,chain)=[old_ptrans old_b1 old_phi old_h];  
    sampled_logLL_1(i,chain)=old_logLL;
end
end

% Calculate acceptance rate
for i=2:total_steps
    accept_rate_1(i,:)=mean(accept(1:i,:));
end

% Only keep values after the burn-in period
for chain=1:2
    keep_pars_1(:,:,chain)=sampled_pars_1(burnin_steps+1:total_steps,:,chain);
end

save('malawi_mcmc_output1.mat','sampled_pars_1','sampled_logLL_1','accept_rate_1','keep_pars_1')

%%
figure
subplot(2,4,1); plot([sampled_pars_1(:,1,1) sampled_pars_1(:,1,2)])
subplot(2,4,2); plot([sampled_pars_1(:,2,1) sampled_pars_1(:,2,2)])
subplot(2,4,3); plot([sampled_pars_1(:,3,1) sampled_pars_1(:,3,2)])
subplot(2,4,4); plot([sampled_pars_1(:,4,1) sampled_pars_1(:,4,2)])
subplot(2,4,5); hist([keep_pars_1(:,1,1) keep_pars_1(:,1,2)])
subplot(2,4,6); hist([keep_pars_1(:,2,1) keep_pars_1(:,2,2)])
subplot(2,4,7); hist([keep_pars_1(:,3,1) keep_pars_1(:,3,2)])
subplot(2,4,8); hist([keep_pars_1(:,4,1) keep_pars_1(:,4,2)])

%%
figure
subplot(2,1,1); plot(sampled_logLL_1)
subplot(2,1,2); plot(accept_rate_1)

%% Gelman-Rubin diagnostic
run_steps=10000;
x=var(keep_pars_1(:,:,1));
x(2,:)=var(keep_pars_1(:,:,2))
y=mean(keep_pars_1(:,:,1));
y(2,:)=mean(keep_pars_1(:,:,2))

grdiag_1=sqrt(((run_steps-1)/run_steps*mean(x) + var(y))./mean(x))
