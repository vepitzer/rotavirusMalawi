function [LL,Dv]=rotafitMV1nr(unkp,pars,dataV,dataU,malpop,Bmal,repeffV,vcov,varargin)
% This is for the base-case model assuming heterogeneity in vaccine response. 
...
priorLL=log(betapdf(unkp(1),19,17))+log(betapdf(unkp(2),32.2,3.8)); %+log(unifpdf(unkp(3),0,1)); %Prior distribution for the probability of responding to each vaccine dose
...
sc2n=(1-sc2)*sc1/(1-sc1); %unkp(3); %sc2; %probability of responding to second dose given did not respond to first dose
sc3n=(1-sc2)^2*sc1/(1-res+res*(1-sc2)^2); %unkp(3); %sc3; %probability of responding to third dose given did not respond to first or second dose
