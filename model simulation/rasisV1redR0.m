function dSt=rasisV1redR0(t,St)
%Differential equations 

global B wm wi1 wi2 u um beta b1 phi1 d1 d2 rr1 rr2 ri2 ri3 al v1 v2 v3 sc1 sc2 sc3 sc2n sc3n wv redR0 reintro wA; 

wv1=wv; 
wv2=wv;
lamda=redR0(round(t))*(1+b1*cos(2*pi*(t-phi1)/52.18))*beta*(St(2*al+1:3*al)+ri2*St(5*al+1:6*al)+ri3*St(8*al+1:9*al)+St(16*al+1:17*al)+ri2*St(19*al+1:20*al)+ri3*St(22*al+1:23*al))/sum(St);
...
for i=1:al
    ...
    dSt(i+17*al,1)=(sc1*v1(round(t),1)+sc2*(1-sc1)*v2(round(t),1))*log(1+B(round(t),i))/52*sum(St(:)) - wv1*St(i+17*al) + d1*St(i+16*al) - (wi1+u(i)+um(i))*St(i+17*al); %dVR0/dt
    ...
    dSt(i+20*al,1)=sc1*sc2*v2(round(t),1)*log(1+B(round(t),i))/52*sum(St(:)) - wv2*St(i+20*al) + d2*St(i+19*al) - (wi1+u(i)+um(i))*St(i+20*al); %dVR1/dt
    ...
    if i>1 %aging of individuals from one age group to the next and vaccination
        ...
        dSt(i+17*al,1)=dSt(i+17*al,1) + (1-sc2*v2(round(t),i))*(1-sc3*v3(round(t),i))*u(i-1)*St(i+17*al-1) + (1-sc1)*v1(round(t),i)*u(i-1)*St(i+3*al-1) + sc1*v1(round(t),i)*u(i-1)*(St(i-1)+St(i+al-1)+St(i+10*al-1)+St(i+11*al-1)+St(i+12*al-1)+St(i+13*al-1)+St(i+14*al-1)) + (sc2n*v2(round(t),i)+sc3n*v3(round(t),i))*u(i-1)*(St(i+15*al-1)+St(i+24*al-1)+St(i+25*al-1)+St(i+26*al-1)+St(i+27*al-1)+St(i+28*al-1)+St(i+29*al-1)); %RV0
        ...
        dSt(i+20*al,1)=dSt(i+20*al,1) + (1-sc2*v2(round(t),i))*(1-sc3*v3(round(t),i))*u(i-1)*St(i+20*al-1) + (1-sc1)*v1(round(t),i)*u(i-1)*St(i+6*al-1) + sc1*v1(round(t),i)*u(i-1)*(St(i+3*al-1)+St(i+4*al-1)) + (sc2*v2(round(t),i)+sc3*v3(round(t),i))*u(i-1)*(St(i+17*al-1)+St(i+18*al-1)); %RV1
        ...
        dSt(i+23*al,1)=dSt(i+23*al,1) + u(i-1)*St(i+23*al-1) + v1(round(t),i)*u(i-1)*St(i+9*al-1) + sc1*v1(round(t),i)*u(i-1)*(St(i+6*al-1)+St(i+7*al-1)) + (sc2*v2(round(t),i)+sc3*v3(round(t),i))*u(i-1)*(St(i+20*al-1)+St(i+21*al-1)); %RV2
