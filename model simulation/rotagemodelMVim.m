%%% MODEL 1 ASSUMING A NEW VACCINE WITH IMPROVED IMMUNOGENICITY IS INTRODUCED IN JANUARY 2018 
...
for im=1:26
sc1=[.687*ones(t0+tvacc+315,1); (.687+(im-1)/100)*ones(tmax-t0-tvacc-315,1)];
sc2=[.687*ones(t0+tvacc+315,1); (.687+(im-1)/100)*ones(tmax-t0-tvacc-315,1)];
sc2n=[.687*ones(t0+tvacc+315,1); (.687+(im-1)/100)*ones(tmax-t0-tvacc-315,1)];
sc3=.687; 
sc3n=sc3;
...
[time St]=ode45('rasisV1im',1:tmax,St0,options); %solve diff eqs
...
for y=1:10
    Hyr0_imprsc1(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:4)));
    Hyr1_imprsc1(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,5:12)));
    Hyr2_imprsc1(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,13:au2)));
    Hyr3_imprsc1(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,au2+1:au5)));
    HyrU5_imprsc1(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:au5)));
end

%Calculate the overall effectiveness for all children <5yo and by age group
OE_imprsc(im,1)=1-sum(HyrU5_imprsc1(7:9,im))./sum(sum(Hyr_nobooster(7:9,:,1),2));
OEage_imprsc1(im,:)=1-[(sum(Hyr0_imprsc1(7:9,im)+sum(Hyr1_imprsc1(7:9,im))))./sum(sum(Hyr_nobooster(7:9,1:2,1),2)) sum(Hyr2_imprsc1(7:9,im))./sum(Hyr_nobooster(7:9,3,1)) sum(Hyr3_imprsc1(7:9,im))./sum(Hyr_nobooster(7:9,4,1))];
