%%% MODEL 2 ASSUMING A NEW VACCINE WITH IMPROVED IMMUNOGENICITY IS INTRODUCED IN JANUARY 2018 
...
for im=1:26
sc1=[.528*ones(t0+tvacc+315,1); (.528+(im-1)/100)*ones(tmax-t0-tvacc-315,1)];
sc2=.895*ones(tmax,1);
res=sc1(1)/sc2(1); %probability of being a "responder"
sc2n=(1-sc2).*sc1./(1-sc1).*ones(tmax,1); %[ones(t0+tvacc+315,1); (1+im/100)*ones(tmax-t0-tvacc-315,1)];
sc3=.895; 
...
for y=1:10
    Hyr0_imprsc2(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:4)));
    Hyr1_imprsc2(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,5:12)));
    Hyr2_imprsc2(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,13:au2)));
    Hyr3_imprsc2(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,au2+1:au5)));
    HyrU5_imprsc2(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:au5)));
end

%Calculate the overall effectiveness for all children <5yo and by age group
OE_imprsc(im,2)=1-sum(HyrU5_imprsc2(7:9,im))./sum(sum(Hyr_nobooster_nr(7:9,:,1),2));
OEage_imprsc2(im,:)=1-[(sum(Hyr0_imprsc2(7:9,im)+sum(Hyr1_imprsc2(7:9,im))))./sum(sum(Hyr_nobooster_nr(7:9,1:2,1),2)) sum(Hyr2_imprsc2(7:9,im))./sum(Hyr_nobooster_nr(7:9,3,1)) sum(Hyr3_imprsc2(7:9,im))./sum(Hyr_nobooster_nr(7:9,4,1))];
