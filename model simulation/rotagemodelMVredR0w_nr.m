%%% MODEL 4 ASSUMING AN INTERVENTION LEADING TO REDUCTION IN R0 
...
for im=1:26
redR0=[ones(t0+tvacc+315,1); (1-(2*im-1)/100)*ones(tmax-t0-tvacc-315,1)];    
...
[time St]=ode45('rasisV1wredR0',1:tmax,St0,options);
...
for t=1:size(St,1)
    lambda(t,:)=redR0(round(t0+t))*(1+b1*cos(2*pi*(time(t)-phi1)/52.18))*((St(t,2*al+1:3*al)+ri2*St(t,5*al+1:6*al)+ri3*St(t,8*al+1:9*al)+St(t,16*al+1:17*al)+ri2*St(t,19*al+1:20*al)+ri3*St(t,22*al+1:23*al))*beta)./sum(St(t,:)); %+b2*cos(2*pi*(time(t)-phi2)/26)
end
...
%popagedist=[sum(popagedist(1:16)) sum(popagedist(17:18)) popagedist(19:al)];
...
for y=1:10
    Hyr0_redR04(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:4)));
    Hyr1_redR04(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,5:12)));
    Hyr2_redR04(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,13:au2)));
    Hyr3_redR04(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,au2+1:au5)));
    HyrU5_redR04(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:au5)));
end

%Calculate the overall effectiveness for all children <5yo and by age group
OE_redR0(im,4)=1-sum(HyrU5_redR04(7:9,im))./sum(sum(Hyr_wane_nr(7:9,:,1),2));
OEage_redR04(im,:)=1-[(sum(Hyr0_redR04(7:9,im)+sum(Hyr1_redR04(7:9,im))))./sum(sum(Hyr_wane_nr(7:9,1:2,1),2)) sum(Hyr2_redR04(7:9,im))./sum(Hyr_wane_nr(7:9,3,1)) sum(Hyr3_redR04(7:9,im))./sum(Hyr_wane_nr(7:9,4,1))];
