...
prop0infVwr=[Mv(:,[3 6 12 18])+St(:,15*al+[3 6 12 18])+St(:,30*al+[3 6 12 18]) sum(Mv(:,au2+1:au5)+St(:,15*al+au2+1:15*al+au5)+St(:,30*al+au2+1:30*al+au5),2)]./[V(:,[3 6 12 18]) sum(V(:,au2+1:au5),2)];
prop1infVwr=[St(:,16*al+[3 6 12 18])+St(:,17*al+[3 6 12 18])+St(:,18*al+[3 6 12 18])+St(:,31*al+[3 6 12 18]) sum(St(:,16*al+au2+1:16*al+au5)+St(:,17*al+au2+1:17*al+au5)+St(:,18*al+au2+1:18*al+au5)+St(:,31*al+au2+1:31*al+au5),2)]./[V(:,[3 6 12 18]) sum(V(:,au2+1:au5),2)];
...
elseif targ==2    
Hrelinf0_wanebooster_nr(:,relinf)=sum(H(:,1:12),2);
Hrelinf1_wanebooster_nr(:,relinf)=sum(H(:,13:au2),2);
Hrelinf2_wanebooster_nr(:,relinf)=sum(H(:,au2+1:au5),2);
end
...
IEtot_homog_wane_nr(relinf,1)=sum((1-(sum(Hu(tvacc+52:tvf-t0,1:au5).*Hu(tvacc+52:tvf-t0,1:au5)./U(tvacc+52:tvf-t0,1:au5),2)./sum(Hu(tvacc+52:tvf-t0,1:au5),2))./(sum(Hnovacc_wane(tvacc+52:tvf-t0,1:au5).*Hnovacc_wane(tvacc+52:tvf-t0,1:au5)./Unovacc_wane(tvacc+52:tvf-t0,1:au5),2)./sum(Hnovacc_wane(tvacc+52:tvf-t0,1:au5),2))).*sum(Hu(tvacc+52:tvf-t0,1:au5),2))/sum(sum(Hu(tvacc+52:tvf-t0,1:au5),2));
IEage_homog_wane_nr(relinf,:)=1-[(sum(sum(Hu(tvacc+52:tvf-t0,1:12),2))/sum(sum(U(tvacc+52:tvf-t0,1:12),2)))/(sum(sum(Hnovacc_wane(tvacc+52:tvf-t0,1:12),2))/sum(sum(Unovacc_wane(tvacc+52:tvf-t0,1:12),2)))...
...
plot(datesim(tvacc:tvf-t0),[Hrelinf0_wane_nr(tvacc:tvf-t0,relinf) Hrelinf1_wane_nr(tvacc:tvf-t0,relinf) Hrelinf2_wane_nr(tvacc:tvf-t0,relinf)],'LineWidth',1.5)
datetick('x','mmm-yy')
