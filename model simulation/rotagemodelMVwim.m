%%% MODEL 3 ASSUMING A NEW VACCINE WITH IMPROVED IMMUNOGENICITY IS INTRODUCED IN JANUARY 2018 
...
sc2=[pvacc(2)*ones(t0+tvacc+315,1); (pvacc(2)+(im-1)/100)*ones(tmax-t0-tvacc-315,1)];
sc2n=[pvacc(2)*ones(t0+tvacc+315,1); (pvacc(2)+(im-1)/100)*ones(tmax-t0-tvacc-315,1)];
sc3=pvacc(2); 
sc3n=sc3;
...
for y=1:10
    Hyr0_imprsc3(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:4)));
    Hyr1_imprsc3(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,5:12)));
    Hyr2_imprsc3(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,13:au2)));
    Hyr3_imprsc3(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,au2+1:au5)));
    HyrU5_imprsc3(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:au5)));
end

%Calculate the overall effectiveness for all children <5yo and by age group
OE_imprsc(im,3)=1-sum(HyrU5_imprsc3(7:9,im))./sum(sum(Hyr_wane(7:9,:,1),2));
OEage_imprsc3(im,:)=1-[(sum(Hyr0_imprsc3(7:9,im)+sum(Hyr1_imprsc3(7:9,im))))./sum(sum(Hyr_wane(7:9,1:2,1),2)) sum(Hyr2_imprsc3(7:9,im))./sum(Hyr_wane(7:9,3,1)) sum(Hyr3_imprsc3(7:9,im))./sum(Hyr_wane(7:9,4,1))];
