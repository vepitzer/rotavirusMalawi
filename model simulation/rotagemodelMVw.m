% Simulates rotavirus model with waning of vaccine-induced immunity and homogeneity in vaccine response
% and estimates vaccine effects for the best-fit parameter values
...
[time St]=ode45('rasisV1w',1:tmax,St0,options); %solve diff eqs
...
prop0infVw=[Mv(:,[3 6 12 18])+St(:,15*al+[3 6 12 18])+St(:,30*al+[3 6 12 18]) sum(Mv(:,au2+1:au5)+St(:,15*al+au2+1:15*al+au5)+St(:,30*al+au2+1:30*al+au5),2)]./[V(:,[3 6 12 18]) sum(V(:,au2+1:au5),2)];
prop1infVw=[St(:,16*al+[3 6 12 18])+St(:,17*al+[3 6 12 18])+St(:,18*al+[3 6 12 18])+St(:,31*al+[3 6 12 18]) sum(St(:,16*al+au2+1:16*al+au5)+St(:,17*al+au2+1:17*al+au5)+St(:,18*al+au2+1:18*al+au5)+St(:,31*al+au2+1:31*al+au5),2)]./[V(:,[3 6 12 18]) sum(V(:,au2+1:au5),2)];
...
elseif targ==2
Hrelinf0_wanebooster(:,relinf)=sum(H(:,1:12),2);
Hrelinf1_wanebooster(:,relinf)=sum(H(:,13:au2),2);
Hrelinf2_wanebooster(:,relinf)=sum(H(:,au2+1:au5),2);
end
...
VEtot_wane(relinf,1)=1-((sum(Hv(tvacc+52:tvf-t0,4:au5))./sum(V(tvacc+52:tvf-t0,4:au5)))*agedistV(4:au5)')/((sum(Hu(tvacc+52:tvf-t0,4:au5))./sum(U(tvacc+52:tvf-t0,4:au5)))*agedistU(4:au5)');
VEage_wane(relinf,:)=1-[(sum(sum(Hv(tvacc+52:tvf-t0,4:12),2))/sum(sum(V(tvacc+52:tvf-t0,4:12),2)))/(sum(sum(Hu(tvacc+52:tvf-t0,4:12),2))/sum(sum(U(tvacc+52:tvf-t0,4:12),2)))...
...
IEtot_wane(relinf,1)=sum((1-(sum(Hu(tvacc+52:tvf-t0,1:au5).*Hu(tvacc+52:tvf-t0,1:au5)./U(tvacc+52:tvf-t0,1:au5),2)./sum(Hu(tvacc+52:tvf-t0,1:au5),2))./(sum(Hnovacc_wane(tvacc+52:tvf-t0,1:au5).*Hnovacc_wane(tvacc+52:tvf-t0,1:au5)./Unovacc_wane(tvacc+52:tvf-t0,1:au5),2)./sum(Hnovacc_wane(tvacc+52:tvf-t0,1:au5),2))).*sum(Hu(tvacc+52:tvf-t0,1:au5),2))/sum(sum(Hu(tvacc+52:tvf-t0,1:au5),2));
IEage_wane(relinf,:)=1-[(sum(sum(Hu(tvacc+52:tvf-t0,1:12),2))/sum(sum(U(tvacc+52:tvf-t0,1:12),2)))/(sum(sum(Hnovacc_wane(tvacc+52:tvf-t0,1:12),2))/sum(sum(Unovacc_wane(tvacc+52:tvf-t0,1:12),2)))...
...
for y=1:10
   if targ==1
        Hyr_wane(y,:,relinf)=[sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:4))) sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,5:12))) sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,13:au2))) sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,au2+1:au5)))];
    elseif targ==2
        Hyr_booster_wane(y,:,relinf)=[sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:4))) sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,5:12))) sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,13:au2))) sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,au2+1:au5)))];
    end
end
...
datesim(1:t0,:)=[]; % delete date values from burn-in period
...
plot(datesim(tvacc:tvf-t0),[Hrelinf0_wane(tvacc:tvf-t0,relinf) Hrelinf1_wane(tvacc:tvf-t0,relinf) Hrelinf2_wane(tvacc:tvf-t0,relinf)],'LineWidth',1.5)
datetick('x','mmm-yy')
ylabel('Number of RVGE cases (per week)')

%% Plot total population and population age distribution through time to verify that it is (roughly) consistent with the observed
%figure; subplot(2,1,1); hold on;
%Population of Blantyre district
%plot(datenum([blantyre_pop(:,1:2) ones(length(blantyre_pop),1) zeros(length(blantyre_pop),3)]),blantyre_pop(:,5),'xb')
%Population of Blantyre city (within Blantyre district)
