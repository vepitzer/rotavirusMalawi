%%% MODEL 1 ASSUMING AN INTERVENTION LEADING TO REDUCTION IN R0 

% NOTE: You need to run rotagemodelMV first
...
for y=1:10
    Hyr0_redR01(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:4)));
    Hyr1_redR01(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,5:12)));
    Hyr2_redR01(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,13:au2)));
    Hyr3_redR01(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,au2+1:au5)));
    HyrU5_redR01(y,im)=sum(sum(H(tvacc+round(52.18*(y-1)):tvacc+round(52.18*(y-1))+51,1:au5)));
end

%Calculate the overall effectiveness for all children <5yo and by age group
OE_redR0(im,1)=1-sum(HyrU5_redR01(7:9,im))./sum(sum(Hyr_nobooster(7:9,:,1),2));
OEage_redR01(im,:)=1-[(sum(Hyr0_redR01(7:9,im)+sum(Hyr1_redR01(7:9,im))))./sum(sum(Hyr_nobooster(7:9,1:2,1),2)) sum(Hyr2_redR01(7:9,im))./sum(Hyr_nobooster(7:9,3,1)) sum(Hyr3_redR01(7:9,im))./sum(Hyr_nobooster(7:9,4,1))];
